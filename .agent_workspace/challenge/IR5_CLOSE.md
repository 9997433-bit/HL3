...
- Sample 15 对官方 `CommandedDisplacementLineCut.xlsx` 做独立 Python 线切割：subset 21 / step 16 / search 16 / VSG 85 px，**RMSE 0.0825 px，bias −0.0345 px**，收敛率 0.636，7198 点。`sample15_k200.json` 只记录该协议；pytest slow 用 step 32 冒烟且 `write=False`，禁止覆盖正式成绩。不是官方 MATLAB 计分器，也不是 VIC 对照。
