...
def test_gate_rejects_matches_pushed_off_their_epipolar_line(rig, frame, matched):
    ...
    # Also locks the (P_left, P_right) argument order: on analytically exact
    # correspondences both residuals must vanish, and they would not if the
    # fundamental matrix were built the other way round.
    ...
    assert float(np.max(clean.distance_px)) < 1e-6
    assert float(np.min(pushed.sampson_px)) > 1.0
    assert float(np.min(pushed.distance_px)) > 2.0
    assert X.shape == (left.shape[0], 3)


def test_the_gate_is_blind_to_a_shift_along_the_epipolar_line(rig, frame, matched):
    """The complement of the previous test, asserted rather than assumed.

    A correspondence slid *along* its epipolar line is a wrong depth with a
    perfect epipolar score -- the failure mode of a match locked onto the
    neighbouring speckle lobe. Neither the gate here nor ZNCC can see it; it
    needs the disparity-surface residual or the four-way loop closure of spec
    section 6.4, both of which come later. Pretending otherwise is how a
    quality field gets trusted past its definition.
    """
    left = matched.left_xy[matched.accepted]
    _, right = _truth(rig, frame, left)
    F = rig_fundamental(rig)
    lines = np.hstack([left, np.ones((left.shape[0], 1))]) @ F.T
    across = lines[:, :2] / np.hypot(lines[:, 0], lines[:, 1])[:, None]
    along = np.column_stack([-across[:, 1], across[:, 0]])

    slid = epipolar_residuals(rig, left, right + 3.0 * along)
    assert float(np.max(slid.sampson_px)) < 1e-6
    assert float(np.max(slid.distance_px)) < 1e-6
...
def test_matching_is_invariant_to_a_gain_and_offset_between_the_cameras(
    images, rig, params, matched
):
    """Two cameras never share a gain, an offset or a vignette; ZNSSD does not care.

    ``g = a f + b`` with ``a > 0`` is exactly the transformation the zero-mean
    normalised criterion is invariant under, and the stereo pair is where that
    invariance earns its keep.
    """
    scaled = 0.6 * images[1] + 37.0
    result = match_stereo_pair(images[0], scaled, rig, params)
    assert np.array_equal(result.accepted, matched.accepted)
    shift = np.linalg.norm(
        result.right_xy[result.accepted] - matched.right_xy[matched.accepted], axis=1
    )
    assert float(np.max(shift)) < 1e-6


def test_a_coincident_pair_is_refused_before_any_correlation_runs(images, params):
    """No baseline, no epipolar geometry -- and no reason to spend the solve."""
    rig = make_stereo_rig(
        baseline_mm=254.0,
        standoff_mm=648.0,
        focal_mm=35.0,
    ...
    coincident = StereoRig(rig.left, rig.left, rig.standoff_mm)
    started = time.perf_counter()
    with pytest.raises(ValueError, match="camera centres coincide"):
        match_stereo_pair(images[0], images[1], coincident, params)
...
def test_masked_correspondences_triangulate_back_onto_the_surface(
    ...
    # The other half of the stage-D gate -- cheirality plus position
    # uncertainty -- lives in the triangulation module and must agree.
    gate = triangulation_quality_mask(
        [rig.left.P, rig.right.P],
        X_est,
        sigma_px=0.05,
        max_position_sigma_mm=0.1,
    )
    assert np.array_equal(gate, keep)
